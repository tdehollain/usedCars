const buildURL = vehicle => {
  ...
  // NEW VERSION
  const result = `${baseURL}/${vehicle.brand}/${
    vehicle.model
  }?sort=price&desc=0&ustate=N%2CU&size=20&page=1${version}${body}${regFrom}${regTo}${fuel}${powerFrom}${powerTo}${gear}${doorFrom}${doorTo}${other}`;
  return result;
};
...
const sortByPropertyScrapDate = (a, b) => {
